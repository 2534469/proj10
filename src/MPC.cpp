...
public:
    ...
    void operator()(ADvector &fg, const ADvector &vars) {
        ...
        for (int t = 1; t < N; t++) {
            ...
            if (t > 1) {
                // Apply actuations one timestap later to account for latency
                // Source: https://github.com/jeremy-shannon/CarND-MPC-Project/blob/master/src/MPC.cpp#L104
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    //only +-25 degrees possible
    for (int i = delta_start; i < a_start; i++) {
        vars_lowerbound[i] = -0.436332;
        vars_upperbound[i] = 0.436332;
    }
    //acceleration/decceleration constraints
